...
Cards accumulate in a per-user deck, synced across your devices once you're
signed in, and export as a tab-separated `.txt` for Anki's File → Import
(built for the `JapaneseFurigana` add-on's field format) — not `.apkg`, so
every card gets reviewed before it reaches your collection.
...
Serves on `http://localhost:8080`. No login by default — sign-in is off
unless a ZITADEL instance is configured (see [AGENTS.md](AGENTS.md)) — and
uses the Claude Agent SDK against your local Claude Code login — no API key
needed, just `ant auth login` (or an existing Claude Code session).
...
Credentials, and requires signing in via ZITADEL — new accounts sit on a
pending page until approved by hand — see [AGENTS.md](AGENTS.md) for the
full backend/auth/storage setup and the organisatrix `infra/README.md`
